...
class NodeFuncCall(Node):
    ...
    def eval(self, env):
        return (env.getFunc(self.pos,self.funcID)).call(env, self.expr.eval(env))

class NodeFuncDecl(Node):
    def __init__(self, funcID, args, expr):
    ...
    def eval(self, env):
        return env.putFunc(self.funcID, Function(self.args, self.expr))
